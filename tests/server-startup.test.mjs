...
for (const [name,account,code] of [
 ...
 test(`serverless startup returns JSON for ${name}`,()=>{
  ...
  const output=execFileSync(process.execPath,['--input-type=module','-e',script],{env:{...process.env,STORE_MODE:'live',VERCEL:'1',FIREBASE_SERVICE_ACCOUNT:account,GOOGLE_APPLICATION_CREDENTIALS:'',FIREBASE_AUTH_EMULATOR_HOST:'',FIRESTORE_EMULATOR_HOST:''},encoding:'utf8',timeout:20000});
...
test('Firebase Admin imports with CommonJS-to-ESM require disabled (Vercel regression)',()=>{
 const output=execFileSync(process.execPath,['--no-experimental-require-module','--input-type=module','-e',"await import('firebase-admin/auth');console.log('ok')"],{encoding:'utf8',timeout:20000});

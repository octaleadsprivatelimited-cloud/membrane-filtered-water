...
export const paymentMode = () => process.env.CASHFREE_MODE || 'disabled';
...
export async function cashfree(path, options={}) {
 ...
 const base=paymentMode()==='production'?'https://api.cashfree.com/pg':'https://sandbox.cashfree.com/pg';
 const response=await fetch(base+path,{...options,signal:AbortSignal.timeout(15000),headers:{'Content-Type':'application/json','x-client-id':process.env.CASHFREE_CLIENT_ID,'x-client-secret':process.env.CASHFREE_CLIENT_SECRET,'x-api-version':process.env.CASHFREE_API_VERSION||'2025-01-01',...options.headers}});

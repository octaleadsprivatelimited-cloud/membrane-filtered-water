...
const admin=(req,res,next)=>req.user.admin===true?next():res.status(403).json({error:'Administrator access required.'});
...
function product(v,existing={}){
 const p={...existing,name:text(v.name,150),description:text(v.description,5000),price:Number(v.price),originalPrice:Number(v.originalPrice||v.price),stock:int(v.stock),image:text(v.image,2000),brand:text(v.brand,100),gtin:text(v.gtin,14),mpn:text(v.mpn,100),features:Array.isArray(v.features)?v.features.map(x=>text(x,200)).filter(Boolean).slice(0,12):[],status:['draft','active','archived'].includes(v.status)?v.status:'draft',condition:['new','used','refurbished'].includes(v.condition)?v.condition:'new',identifiersExist:v.identifiersExist!==false,merchantEnabled:v.merchantEnabled===true,demo:v.demo===true,updatedAt:new Date().toISOString()};
 ...
 if(!/^https?:\/\//.test(p.image)&&!/^\/[\w./-]+$/.test(p.image))fail('Use a valid image URL or local asset path.');
...
async function settlePayment(orderId) {
 ...
 if(remote.order_status==='PAID')await db.runTransaction(async tx=>{const fresh=await tx.get(ref);if(fresh.data().paymentStatus!=='paid')tx.update(ref,{paymentStatus:'paid',status:'confirmed',paidAt:new Date().toISOString()});});
...
app.use((req,res,next)=>{res.set('X-Content-Type-Options','nosniff');if(req.path.startsWith('/api/'))res.set('Cache-Control','no-store');if(!['GET','HEAD','OPTIONS'].includes(req.method)&&req.headers.origin){const allowed=emulator?['http://127.0.0.1:5173','http://localhost:5173']: [process.env.PUBLIC_STORE_URL];if(!allowed.includes(req.headers.origin))return res.status(403).json({error:'Origin not allowed'});}next();});
...
app.get('/api/me',authenticated,async(req,res)=>{const ref=db.doc(`customers/${req.user.uid}`);const snap=await ref.get();if(!snap.exists)await ref.set({email:req.user.email||'',name:req.user.name||'',addresses:[],createdAt:new Date().toISOString()});res.json({...(await ref.get()).data(),uid:req.user.uid,admin:req.user.admin===true});});
...
app.post('/api/orders',authenticated,async(req,res)=>{
 ...
 const subtotalPaise=items.reduce((n,p)=>n+Math.round(p.price*100)*p.quantity,0);const shippingPaise=settings.freeShippingAbove>0&&subtotalPaise>=settings.freeShippingAbove*100?0:Math.round(settings.shippingFee*100);
 if(method==='cashfree'&&subtotalPaise+shippingPaise<100)fail('Cashfree requires an order total of at least ₹1.');
 const order={id:orderId,uid:req.user.uid,email:req.user.email||'',items,address:shipping,subtotalPaise,shippingPaise,totalPaise:subtotalPaise+shippingPaise,currency:'INR',paymentMethod:method,paymentStatus:method==='demo'?'demo':'pending',status:method==='demo'?'confirmed':'pending_payment',createdAt:new Date().toISOString(),requestHash,tracking:'',paymentKey:randomUUID()};
...
app.patch('/api/admin/orders/:id',async(req,res)=>{const ref=db.doc(`orders/${id(req.params.id)}`);const next=req.body.status;await db.runTransaction(async tx=>{const s=await tx.get(ref);if(!s.exists)fail('Order not found',404);const o=s.data();const transitions={confirmed:['processing'],processing:['shipped'],shipped:['delivered']};if(next!==o.status&&!transitions[o.status]?.includes(next))fail('Invalid order status transition',409);tx.update(ref,{status:next,tracking:text(req.body.tracking,200),updatedAt:new Date().toISOString()});});res.json({ok:true});});
app.get('/api/admin/customers',async(req,res)=>res.json(await all('customers')));
app.put('/api/admin/settings',async(req,res)=>{const b=req.body;const siteUrl=text(b.siteUrl,500).replace(/\/$/,'');if(siteUrl&&!publicHttps(siteUrl))fail('Use a public HTTPS domain');const s={siteUrl,shippingFee:Number(b.shippingFee),freeShippingAbove:Number(b.freeShippingAbove),supportEmail:text(b.supportEmail,200),businessName:text(b.businessName,150),merchantLive:b.merchantLive===true};for(const key of ['shippingPolicy','returnsPolicy','privacyPolicy','termsPolicy'])s[key]=text(b[key],10000);if(!Number.isFinite(s.shippingFee)||s.shippingFee<0||s.shippingFee>100000||!Number.isFinite(s.freeShippingAbove)||s.freeShippingAbove<0)fail('Invalid shipping rates');if(s.merchantLive&&(!siteUrl||!s.supportEmail||['shippingPolicy','returnsPolicy','privacyPolicy','termsPolicy'].some(k=>!s[k])||emulator||!cashfreeReady()||paymentMode()!=='production'))fail('Live feed requires live Firebase, production payments, public domain, contact and all policies.');await db.doc('settings/store').set(s);res.json(s);});
